...
Agent 模式底层使用 Claude Agent SDK，因此目前要求渠道提供 Anthropic 或 Anthropic 兼容协议。Chat 模式则通过 `@legis/core` 的 Provider Adapter 统一接入不同协议。
...
| 包 | 版本 | 职责 |
| --- | --- | --- |
| `@legis/electron` | `0.10.7` | Electron 桌面应用 |
| `@legis/shared` | `0.1.20` | 共享类型、IPC 常量、配置和工具 |
| `@legis/core` | `0.2.9` | Provider Adapter、SSE、Shiki 高亮 |
| `@legis/ui` | `0.1.6` | 共享 React UI 组件 |
