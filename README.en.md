...
Agent mode is powered by Claude Agent SDK, so it currently requires an Anthropic or Anthropic-compatible channel. Chat mode uses Provider Adapters from `@runwork/core` to support different protocols.
...
| --- | --- | --- |
| `@runwork/electron` | `0.9.12` | Electron desktop app |
| `@runwork/shared` | `0.1.17` | shared types, IPC constants, config, utilities |
| `@runwork/core` | `0.2.9` | Provider Adapters, SSE, Shiki highlighting |
| `@runwork/ui` | `0.1.3` | shared React UI components |
