# 审查记录：2026-07-03 品牌重命名 @proma → @runwork

> 审查对象：[`docs/change/2026-07-03-brand-rename-proma-to-runwork.md`](../change/2026-07-03-brand-rename-proma-to-runwork.md)
> 对应提交：`aeb9f3a1 refactor: 品牌重命名 @proma → @runwork`
...
| 文档声明 | 核实结果 |
|---|---|
| 6 个 `package.json` name 全部改为 `@runwork/*` | ✅ root + 5 子包，无残留 `@proma` 包名 |
| CLI bin name `proma → runwork` | ✅ `apps/cli/package.json` 已是 `"runwork"` |
| `--filter='@runwork/electron'` | ✅ root package.json 已改 |
| `PROMA_DEV → RUNWORK_DEV`、`PROMA_CLI → RUNWORK_CLI` | ✅ config-paths.ts / agent-orchestrator.ts / paths.ts 全部落地 |
| `appId / productName` 改动 | ✅ 落地 |
| 菜单栏"关于 RunWork / 隐藏 RunWork / 退出 RunWork" | ✅ menu.ts 已改 |
| AboutSettings GitHub URL → `shanyu910/RunWork-pro` | ✅ AboutSettings.tsx 已改 |
...
文档第 32 行明确写："Agent 系统提示词：`你是 Proma Agent` → `你是 RunWork Agent`"。但实际代码中，**面向 Agent 的系统提示词里仍大量使用 "Proma CLI"**：
...
**关键矛盾**：CLI 的实际 bin name 已经叫 `runwork`（`@runwork/cli`），打包注入的环境变量是 `RUNWORK_CLI`，prompt builder 里组装的命令字符串也用 `"$RUNWORK_CLI"`（`agent-orchestrator.ts:282`）。也就是说**Agent 看到的提示词说 "Proma CLI"，但实际执行的是 `runwork` 命令**——这不仅是品牌不一致，更可能让模型产生混淆。

**建议**：把这些系统提示词里的 "Proma CLI" 统一改为 "RunWork CLI"（推荐），或在文档"保留不改"清单里显式列出并说明理由。
...
这是一个**明显未完成的占位 URL**——`yourusername/proma` 既不是合法的 fork 地址，也和 AboutSettings 里指向的 `shanyu910/RunWork-pro` 不一致。文档完全未提及。这属于明显的遗漏 bug，应当修复并补进文档。
...
下载链接指向旧品牌站点，对 RunWork 用户是误导。`api.proma.cool`（更新检查后端）保留可以理解，但 `proma.cool/download` **是面向用户的下载入口**，不属于"原作者 API 服务"。建议文档把这两类区分开，并考虑把 `/download` 链接也改为 RunWork 自己的发布页。
...
错误提示里的"反馈 issues"链接还指向上游 `ErlichLiu/Proma`，会把 RunWork 用户的 bug 报告送到原作者仓库。文档完全未提及。应当被显式记录——要么改成 `shanyu910/RunWork-pro/issues`（与 AboutSettings 一致），要么在文档里明确"保留指向上游"的决策及理由。
...
```
PromaLogoSettings.tsx:18-35   import promaBlackLogo from '@/assets/bots/runwork-logos/proma-black.png'
                              （14 个 logo 变量）
AppearanceSettings.tsx:39-51  同样 14 个 proma*Logo import
```

目录已改名 `runwork-logos/`，但**图片文件名和 JS 变量名仍带 `proma` 前缀**。这是合理的"内部标识符可保留"情形，但数量很大（光这两个文件就贡献了 71 处 proma 匹配），文档"保留不改"清单却**完全没提到**。对审核者会造成困惑——"791 处 proma 残留到底都是啥？"。

**建议**：在"保留不改"清单补一行：

| 标识符 | 原因 |
|---|---|
| Logo 文件名 `proma-*.png` 及 import 变量 `proma*Logo` | 纯内部资源标识，用户不可见；目录已改为 `runwork-logos/` |
...
# 核实 env var 改名
grep -rn "RUNWORK_CLI\|PROMA_CLI\|RUNWORK_DEV\|PROMA_DEV" apps packages \
