# 审查记录：2026-07-03 RunWork 品牌定制系列变更（联合审查）
...
| # | 文档 | 对应提交 | 详见 |
|---|---|---|---|
| 1 | 品牌重命名 `@proma → @runwork` | `aeb9f3a1` | [单独审查报告](./2026-07-04-brand-rename-proma-to-runwork-review.md) |
| 2 | 法务风格主题 `legal-dark` | `7cfa448c` | 本文档 |
| 3 | RunWork 品牌定制总览 | （聚合） | 本文档 |
...
| 分支 `origin/shanyu910/Proma`，含 `main` + `RunWork` | ✅ remote 与 branch 验证一致 |
| 4 份子文档链接 | ✅ 全部存在 |
...
- `resources/runwork-logos/`：含 **14 个** `proma-*.png` 变体（+ `icon.svg` + 3 个 `iconTemplate*`，共 18 项）
- `renderer/assets/bots/runwork-logos/`：**14 个** `proma-*.png` 变体
...
> - `icon.svg` 源文件：未改动（RunWork 和 Proma 的 svg 源相同）。

实际文件分布与文档暗示不符：

- 托盘图标 `iconTemplate*.png`（3 个）**实际位于 `resources/runwork-logos/` 目录内**，而非 `resources/` 根目录。文档把它们和主图标混在"保留的部分"里，容易让人误以为它们在根目录且未参与目录重命名——实际上它们是随 `proma-logos → runwork-logos` 一起被移动的。
- `icon.svg` **同时存在两处**：`resources/icon.svg`（根，1560 字节）和 `resources/runwork-logos/icon.svg`（logos 目录内）。文档只提"未改动"，没说明这两份及其与图标生成链路（`generate-icons.sh`）的关系。

**建议**：明确写出 `iconTemplate*` 和 `icon.svg` 都位于 `runwork-logos/` 目录内（随目录重命名移动），并说明它们是托盘图标与图标生成链路的源文件。
...
| 目录重命名 `proma-logos → runwork-logos`（两处） | ✅ 两个旧目录均不存在，新目录存在 |
| `tray.ts` 路径同步 `proma-logos → runwork-logos` | ✅ `tray.ts:26-27` 已改 |
| `electron-builder.yml` extraResources 路径 | ✅ `from: resources/runwork-logos` / `to: runwork-logos` |
...
| 配置路径 `~/.runwork/settings.json` | ✅ 与 brand-rename 后的目录命名一致 |
...
# icon-replacement 核验
ls apps/electron/resources/runwork-logos/ | wc -l            # 18（含 14 变体 + icon.svg + 3 iconTemplate）
ls apps/electron/src/renderer/assets/bots/runwork-logos/ | wc -l   # 14
grep -n "runwork-logos" apps/electron/src/main/tray.ts
