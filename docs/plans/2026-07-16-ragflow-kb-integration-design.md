...
└─────┬─────┘  └──────────────┘  └──────────────┘  └──────────────┘
   ...
   ← 所有服务共用 SSO token（Legis-Server 签发，共享密钥验证）→
```
...
| 解析服务 | 无状态 | 无 | — | 被知识库调 |
| 脱敏服务 | 无状态 | 无 | — | 被知识库调 |
| Legis-Server | 已有 | 已有 | — | 签发 SSO token |
...
Legis-Server（已有，改一处配置）
  ...
  JWT payload: { user_id, email, tenants[], active_tenant_id, exp }
       │
       │  共享 SHARED_SECRET
       ▼
知识库服务 / 项目管理服务（新建）
  收到请求 → 用 SHARED_SECRET 验 JWT → 信任 user_id → 查权限
  不需要再登录，不需要 token 交换
```

**用户视角**：登录 Proma（照旧）→ 打开 Agent 会话或知识库管理面板 → 自动可用。全程一次登录，零额外配置。

**JWT 过期处理**：Proma 已有 token 续签机制，复用。
