...
| 8 | 认证 | SSO 共享密钥（改 Legis-Server 一处） | 用户一次登录，token 自动复用 |
...
| 18 | 账号同步 | 实时查 Legis-Server + 轻量缓存（TTL 5min）| 单一数据源，知识库服务不存用户表，砍掉 RAGFlow 的 User/Tenant/UserTenant |
...
└─────┬─────┘  └──────────────┘  └──────────────┘  └──────────────┘
   ...
   ← 认证: 所有服务共用 SSO token（Legis-Server 签发，共享密钥验证）
   ← 账号: 知识库/项目服务实时查 Legis-Server 拿团队/角色（单一数据源，见 §7）→
```
...
| 解析服务 | 无状态 | 无 | — | 被知识库调 |
| 脱敏服务 | 无状态 | 无 | — | 被知识库调 |
| Legis-Server | 已有 | 已有 | — | SSO 签发 + 账号权威源（团队/角色实时查询）|
...
| **账号同步（Provisioning）** | "你的团队/角色/权限数据在哪" | 实时查 Legis-Server + 轻量缓存 |

### 7.1 认证层：SSO 共享密钥

```
Legis-Server（已有，改一处配置）
...
**用户视角**：登录 Legis（照旧）→ 打开 Agent 会话或知识库管理面板 → 自动可用。全程一次登录，零额外配置。

**JWT 过期处理**：Legis 已有 token 续签机制，复用。

### 7.2 账号同步层：实时查 Legis-Server

**问题本质**：SSO 只解决了"user_id 是谁"，但知识库服务的数据库里，需要知道这个 user_id 属于哪些团队、什么角色、能访问哪些知识库。RAGFlow 原生有 `User/Tenant/UserTenant` 三张表管这些，但和 Legis-Server 的账号数据会双份冲突。

**方案：单一数据源 + 实时查询**

知识库服务**不存用户表**，每次请求时用 JWT 里的 user_id 去 Legis-Server 查团队和角色，带短时缓存。

```
Legis 桌面端（带 JWT: user_id=zhangsan）
    │
    ▼
知识库服务收到请求
    │
...
**Legis-Server 新增内部接口**（服务间调用，不走用户 JWT）：
...
```
RAGFlow 原 Tenant 表:
  id, name                         ← 团队信息（改为从 Legis-Server 实时查）
  llm_id, embd_id, rerank_id ...  ← 模型配置（全局单一，砍掉）

改造后:
  知识库服务不再需要 User / Tenant / UserTenant 三张表
  用户/团队/角色 → 实时从 Legis-Server 查（单一数据源）
  模型配置 → 全局环境变量/配置文件
```

**本地只保留团队名称映射**（可选，用于知识库列表显示团队名而不用每次查 Legis-Server）：

```
本地表 kb_tenant_names（轻量缓存，非权威数据）:
  tenant_id (PK), name, updated_at
  ← 从 Legis-Server 的返回里顺带刷新

Knowledgebase 表直接引用 Legis-Server 的 tenant_id:
  Knowledgebase.tenant_id = "team_law1"   ← Legis-Server 的团队 ID
```
...
② 查 Legis-Server（带缓存）→ zhangsan 属于 [team_law1(owner), team_law2(viewer)]
③ 查知识库 kb_abc:
...
| | 实时查 Legis-Server | JWT 携带权限 | 定期同步副本 |
|---|---|---|---|
| 单一数据源 | ✅ 是 | ✅ 是 | ❌ 双份 |
...
账号同步: 知识库/项目服务不存用户表，实时查 Legis-Server（见 §7.2）。
```
