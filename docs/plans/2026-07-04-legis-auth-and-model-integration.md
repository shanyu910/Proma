...
所有 Legis 自定义代码集中在 `apps/electron/src/legis/`，原项目只做 5 处最小挂载。

```
apps/electron/src/
├── legis/                           ← 所有 Legis 代码（独立领地）
│   ├── index.ts                     ← 对外统一出口
│   │
│   ├── auth/                        ← 登录认证
...
│   │
│   ├── config/                      ← Legis 配置管理
│   │   └── legis-config.ts          ← ~/.legis-dev/legis-config.json 读写
│   │
│   ├── settings/                    ← 设置页 UI
...
│   └── lib/                         ← （legis/secure/ 的代码被 ipc.ts import）
│
├── preload/
...
**额外文件**（非 legis/ 目录内）：
- `apps/electron/.env` — 环境变量（VITE_LEGIS_SERVER_URL，不提交 git）
...
```tsx
import { LoginModal, AuthInitializer } from './legis'   // +1 import
...
```typescript
// main/ipc.ts（+15 行）
import { handleSecureTokenGet, handleSecureTokenSet, handleSecureTokenClear } from './lib/legis/auth-secure-storage'
...
case "model-management":
  return <ModelManagementPanel />   // 从 legis/ import
```

### 挂载点 4：GeneralSettings.tsx（+2 行）

```tsx
import { AccountInfoCard } from '../../legis'   // +1
...
```tsx
// ChatInput.tsx
import { useRequireAuth } from '../../legis'    // +1
...
启动 → GET /auth/me/model-config → SK 存模块级变量（非 atom）
  ↓
channels.json 的 apiKey 字段写占位符 "__LEGIS_INJECT__"
  ↓
Chat/Agent 调模型前：
...
```typescript
// legis/model/channel-sync.ts
import type { ModelConfig } from '../types'

const OFFICIAL_CHANNEL_ID = 'legis-official'
...
export async function syncToChannels(config: ModelConfig): Promise<void> {
  ...
  const officialChannel = {
    ...
    apiKey: '__LEGIS_INJECT__',        // 占位符，真 SK 在内存
...
// 在 channel.apiKey === '__LEGIS_INJECT__' 时，用 getSK() 替换
export function resolveApiKey(channelApiKey: string): string | null {
  if (channelApiKey === '__LEGIS_INJECT__') {
    return skInMemory
  }
...
| 步骤 | 内容 | 文件 | 改原项目 |
|---|---|---|---|
| 1 | 类型定义 | `legis/types.ts` | 否 |
| 2 | Keychain 安全存储 + IPC | `legis/secure/auth-secure-storage.ts` + `ipc.ts` + `preload` | ✅ 挂载点 2 |
| 3 | 认证 API + 状态 | `legis/auth/auth-api.ts` + `auth-state.ts` | 否 |
| 4 | model-config + SK 管理 + channel-sync | `legis/model/*` | 否（运行时改 channels.json） |
| 5 | model-usage 余额 | `legis/model/model-usage.ts` | 否 |
| 6 | 登录弹窗 + AuthInitializer + useRequireAuth | `legis/auth/*` | 否 |
| 7 | App.tsx 挂载 LoginModal + AuthInitializer，移除旧硬关卡 | `App.tsx` | ✅ 挂载点 1 |
| 8 | 设置页：ModelManagementPanel + AccountInfoCard | `legis/settings/*` + `legis/account/*` + `SettingsPanel.tsx` + `GeneralSettings.tsx` | ✅ 挂载点 3、4 |
...
| 上游改动场景 | 冲突难度 | 说明 |
|---|---|---|
| 新增 Skill / 修 bug / 加非渠道功能 | 🟢 容易 | legis/ 完全不涉及 |
...
```bash
# apps/electron/.env
# Legis-Server 认证服务地址（登录、验证 Token、拉配置）
VITE_LEGIS_SERVER_URL=http://10.167.1.251:31006
```
...
# Legis-Server 认证服务地址
VITE_LEGIS_SERVER_URL=http://your-legis-server:31006
```
...
const SERVER_URL = import.meta.env.VITE_LEGIS_SERVER_URL || 'http://10.167.1.251:31006'
```

**注意**：AgentSkill 的模型调用地址（baseUrl）**不写进 .env**——文档明确要求从 `/auth/me/model-config` 的 `provider.baseUrl` 动态获取。`.env` 里只放 Legis-Server 地址这一个"入口地址"。

### 12.2 用户配置（运行时可变）→ JSON 文件

模型勾选、默认模型这类**每个用户不同、可随时改**的配置，用独立的 JSON 文件。

**文件位置**：`~/.legis/legis-config.json`（正式版）或 `~/.legis-dev/legis-config.json`（开发模式）
...
- Legis 模块代码独立，配置也应独立（符合与上游最小冲突原则）
- 自包含——将来删除 Legis 改动，删掉 `legis-config.json` 即可，不影响原项目配置

### 12.3 完整配置清单

| 配置项 | 位置 | 方式 | 谁维护 |
|---|---|---|---|
| Legis-Server 地址 | `apps/electron/.env` | 环境变量（VITE_ 前缀） | 开发者（打包时定） |
| AgentSkill 地址 | 不存 | 从 model-config 动态获取 | 服务端 |
| SK（API Key） | 仅内存 | 模块级私有变量 | 运行时 |
| Token | Keychain | safeStorage | 系统级 |
| 模型勾选 | `~/.legis-dev/legis-config.json` | JSON 文件 | 用户 |
| 默认模型 | `~/.legis-dev/legis-config.json` | JSON 文件 | 用户 |

### 12.4 配置数据流

```
apps/electron/.env（构建期）
  VITE_LEGIS_SERVER_URL
       ↓
legis/ 模块启动时读取
       ↓
连接 Legis-Server → 登录 → 拉取 model-config
       ↓                                    ↓
SK 存内存（不写磁盘）              provider.models 存 atom
       ↓                                    ↓
Chat/Agent 调模型时用              用户勾选 → ~/.legis-dev/legis-config.json
```
...
```
服务地址：http://10.167.1.251:31006（Legis-Server）
...
| 步骤 | 内容 | 文件 | 改原项目 |
|---|---|---|---|
| 1 | 类型定义（用第 14 节修正版） | `legis/types.ts` | 否 |
| 2 | .env 配置 + .env.example | `apps/electron/.env` + `.env.example` | 新增文件 |
| 3 | Keychain 安全存储 + IPC | `legis/secure/auth-secure-storage.ts` + `ipc.ts` + `preload` | ✅ 挂载点 2 |
| 4 | 认证 API + 状态 | `legis/auth/auth-api.ts` + `auth-state.ts` | 否 |
| 5 | model-config + SK 管理 + channel-sync | `legis/model/*` | 否（运行时改 channels.json） |
| 6 | model-usage 余额 | `legis/model/model-usage.ts` | 否 |
| 7 | legis-config.json 读写 | `legis/config/legis-config.ts` | 否 |
| 8 | 登录弹窗 + AuthInitializer + useRequireAuth | `legis/auth/*` | 否 |
...
- [x] 配置方案：.env（服务地址）+ legis-config.json（用户偏好）
- [x] 接口验证：6 个接口全部通过
