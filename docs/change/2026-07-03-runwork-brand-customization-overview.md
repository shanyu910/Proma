# 2026-07-03 RunWork 品牌定制总览

> 基于 Proma v0.14.2（SDK 0.3.197）fork，在 `RunWork` 分支上完成品牌定制改造。
...
| # | 改动 | 提交 | 说明 |
|---|---|---|---|
| 1 | 品牌重命名 `@proma → @runwork` | `aeb9f3a1` | 全量包名、配置、文案重命名 |
| — | session-mirror 前缀修复 | `d090d8e8` | brand-rename 的补丁（群名前缀遗漏导致测试失败） |
| 2 | 替换应用图标为 RunWork 品牌 | `9d1ef5ac` | 天平+盾牌法务主题图标 |
| 3 | 登录认证系统 | `359e4147` | 远端认证（配置化优化版） |
| — | 登录后白屏修复 | `507ae8b4` | auth 回归修复（App.tsx Hooks 顺序违规） |

> **已撤销**：法务风格主题（`7cfa448c`）已于 2026-07-04 回退，保留原有主题系统不变。

**详细文档**：
- [品牌重命名](./2026-07-03-brand-rename-proma-to-runwork.md)
- [图标替换](./2026-07-03-logo-and-icon-replacement.md)
- [登录认证系统](./2026-07-03-remote-auth-system.md)

**审查报告**（change ↔ check 双向追溯）：
- [品牌重命名审查](../check/2026-07-04-brand-rename-proma-to-runwork-review.md)
- [品牌定制联合审查](../check/2026-07-04-runwork-brand-customization-review.md)
...
| `settings.ts` | 新增 authServerUrl | 两边的新增字段都要保留 |
| `electron-builder.yml` | appId/productName/扩展名 | 保留你的品牌配置 |

**几乎不会冲突的改动**：
- 所有 `@proma → @runwork` 的 import 替换（上游新增文件用 `@proma`，sed 批量改即可）
- 图标二进制资源（直接用你的版本）
