# Legis 对 Proma 的完整修改总结

> 日期：2026-07-06
> 分支：`Legis`（基于 `main` 的 `3e6f1d14`）
> 用途：归档 Legis 分支相对于上游 Proma 的全部改动，便于团队理解和后续上游同步
...
| **新增核心模块** | `legis/` 21 个文件，2,658 行 |

### 代码分布

| 目录 | 文件数 | 行数变化 |
|------|--------|---------|
| `legis/` 模块（新增核心定制） | 21 | +2,658 |
| `renderer/`（修改的 UI 组件，非 legis） | 135 | +719 / -1,692 |
...
- **包名**：`@proma/shared` → `@legis/shared`，`@proma/electron` → `@legis/electron` 等
- **路径别名**：`~/.proma/` → `~/.legis/`（开发模式 `~/.legis-dev/`）
- **UI 文案**：所有用户可见的 "Proma" 字样改为 "Legis"
...
| **图标生成脚本** | 修复 `generate-icons.sh` 路径 bug（`proma-logos` → `legis-logos`） |
...
- `resources/legis-logos/icon.svg` — 纯黑 L 剪影（派生托盘 Template）
- `resources/legis-logos/iconTemplate{,@2x,@3x}.png` — macOS 托盘多分辨率
- `src/renderer/assets/models/legis.png` — 渠道标题行 logo（256×256）

### 🔐 3. 完整登录/认证系统（新增 `legis/` 模块，21 个文件）

这是**最核心的新增功能**，对接 Legis-Server 后端。
...
| `config/` | legis-config | 用户偏好（模型勾选、默认模型）|
...
- **Token** 用 Electron `safeStorage` AES-256-GCM 加密存 `~/.legis/auth-token.enc`
- 渲染进程通过 `LEGIS_SK_PLACEHOLDER = '__LEGIS_INJECT__'` 占位符 + 主进程 `decryptApiKey` 拦截
...
登录 → fetchModelConfigData（拉 SK + 模型列表）
     ...
     → upsertOfficialChannel（固定 ID 'legis-official'，避免 UUID 重复）
     → refreshStateAfterLogin（刷新 4 个 atom，免重启）
        ...
        └─ legisConfigAtom       ← ModelSelector 过滤
```
...
1. **官方渠道 UUID 重复** → 用固定 ID `legis-official`
2. **模型勾选后 ModelSelector 不刷新** → 加 `refreshChannels()`
3. **登录后必须重启才能看到模型** → 加 `refreshStateAfterLogin`（刷新 channelsAtom + agentChannelIdAtom + legisConfigAtom）
4. **重启后显示所有模型** → ModelSelector 按 `selectedModelIds` 二次过滤官方渠道
5. **显示 Claude 图标** → `getChannelLogo` 加 `legis-official` 特判
...
| `legis-config.test.ts` | 11 | 配置读写、模型勾选 |
...
| 指标 | 状态 |
|------|------|
| Legis 基于 main 的 commit | `3e6f1d14`（fix agent mcp discovery scope） |
| main 当前最新 | `e51ac144`（fix input plain text copy） |
| **未合并的上游提交** | **29 个** |

**最小挂载点设计**带来的好处 —— 80% 的定制集中在独立的 `legis/` 模块里，与上游 Proma 代码物理隔离。但 278 个文件的品牌重命名改动在合并上游时可能需要手工解决冲突。

### 上游同步策略

- `legis/` 模块：零冲突（独立目录）
- 品牌重命名文件（`@legis/*` 路径）：可能冲突，需手工合并
...
2. **最小挂载点**：Legis 定制代码集中在 `legis/` 模块，对上游文件改动最小化
...
| `src/legis/index.ts` | Legis 模块统一导出 |

### 核心模块

| 文件 | 作用 |
|------|------|
| `legis/auth/AuthInitializer.tsx` | 启动时静默验证 Token |
| `legis/auth/LoginModal.tsx` | 登录弹窗 |
| `legis/auth/auth-api.ts` | 登录/验证/改密 API |
| `legis/auth/useAuthGate.ts` | 权限守卫 Hook |
| `legis/secure/auth-secure-storage.ts` | Keychain + SK 主进程内存 |
| `legis/model/model-config.ts` | model-config 拉取 + SK 同步 |
| `legis/model/channel-sync.ts` | model-config → channels.json 同步 |
| `legis/model/post-login-refresh.ts` | 登录后刷新全部相关 atom |
| `legis/settings/ModelManagementPanel.tsx` | 模型管理设置页 |
| `legis/account/AccountMenu.tsx` | 侧边栏账号菜单 |

### 主进程对接

| 文件 | 改动 |
|------|------|
| `src/main/ipc.ts` | 注册 Legis IPC 通道（auth-secure / legisSK / legisChannel） |
| `src/main/lib/channel-manager.ts` | `upsertOfficialChannel` + `decryptApiKey` SK 拦截 |
| `src/preload/index.ts` | 暴露 `authSecure` / `legisSK` / `legisChannel` API |

### 配置文件

| 文件 | 说明 |
|------|------|
| `apps/electron/.env` | `VITE_LEGIS_SERVER_URL`（gitignored，本地配置） |
...
```
VITE_LEGIS_SERVER_URL=http://10.167.1.251:31006   # 测试环境
VITE_LEGIS_SERVER_URL=http://14.103.216.135:31006 # 生产环境
```
...
```
8162b30d feat: 替换应用图标为 Legis 蓝 L 设计
...
8511833f test: 为 Legis 模块补充单元测试（4 个文件，39 个测试）
fe9c3aa6 fix: 修复 7 个问题（功能缺失 + 代码冗余 + 死配置）
c09f073d docs: 新增 Legis 合并文档（2026-07-06）
2bd2fb81 fix: Legis-Server 地址改为 14.103.216.135:31006
...
7be0b34a feat(legis): 修改密码功能完整实现
54b98448 fix: 账号菜单用 Radix Popover 替代手动定位，确保往上弹出
66bf21f4 refactor(legis): 账号信息从设置页移到侧边栏弹出菜单（方案B）
3317fe86 fix: 购买余量链接改为正确地址 31003
7352f78c fix: 去掉输入框的'登录后开始对话'placeholder
8288bb15 fix(legis): 修复模型勾选后 ModelSelector 不刷新
ad5707df fix(legis): 模型勾选同步到渠道 + 初始化从渠道读取勾选
2c93e014 fix(legis): 修复官方渠道 UUID 重复问题 + 模型选择器找不到渠道
220bdf30 fix(legis): SK 主进程内存打通 + 登录后切换官方渠道
e6818340 feat(legis): 未登录降级为只读浏览模式
89d00298 feat(legis): 步骤10 Chat/Agent requireAuth（挂载点5）
6c9c6ae4 feat(legis): 步骤9 App.tsx挂载 + 设置页（挂载点1/3/4）
fb97fb40 feat(legis): 步骤6-8 余额+配置+登录弹窗+认证初始化
535737b1 feat(legis): 步骤4-5 认证API + SK管理 + 渠道同步
5f586cce feat(legis): 步骤1-3 类型定义 + .env配置 + Keychain安全存储
