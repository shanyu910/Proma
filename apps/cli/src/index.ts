 ...
 *
 * 用法：
 *   legis <command> [subcommand] [args] [--flags]
 *   legis session list|info|outline|search|export ...
 *
 * 全局 flag：
 *   --json            输出机器可读 JSON
 *   --config-dir DIR  指定 Legis 配置目录（默认 ~/.legis，LEGIS_DEV=1 → ~/.legis-dev）
 *   --dev             使用 ~/.legis-dev
 *
...
function printHelp(): void {
  info('legis — Legis 会话渐进式读取 CLI\n')
  info('用法: legis session <command> [args] [--flags]\n')
  info('命令:')
...
async function main(): Promise<number> {
  ...
  // 命名空间：当前只有 session，支持 `legis session <cmd>` 与直接 `legis <cmd>`
  let rest = argv
