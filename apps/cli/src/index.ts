#!/usr/bin/env bun
/**
 * runwork — RunWork 命令行工具入口。
 *
 * 用法：
 *   runwork <command> [subcommand] [args] [--flags]
 *   runwork session list|info|outline|search|export ...
 *
 * 全局 flag：
 *   --json            输出机器可读 JSON
 *   --config-dir DIR  指定 RunWork 配置目录（默认 ~/.runwork，RUNWORK_DEV=1 → ~/.runwork-dev）
 *   --dev             使用 ~/.runwork-dev
 *
...
function printHelp(): void {
  info('runwork — RunWork 会话渐进式读取 CLI\n')
  info('用法: runwork session <command> [args] [--flags]\n')
  info('命令:')
...
async function main(): Promise<number> {
  ...
  // 命名空间：当前只有 session，支持 `runwork session <cmd>` 与直接 `runwork <cmd>`
  let rest = argv
