...
// 会话转录的纯逻辑(Turn 分组 / 快照去重 / 预览)已下沉到 @runwork/session-core 作为唯一真源。
...
import {
  ...
  type MessageGroup,
  type AssistantTurn,
} from '@runwork/session-core'
export { groupIntoTurns, getGroupPreview, extractUserText } from '@runwork/session-core'
export type { MessageGroup, AssistantTurn } from '@runwork/session-core'
...
import type { AgentPendingFile } from '@runwork/shared'
import {
  getSDKCompactStatus,
...
// extractMeta / MessageMeta 已迁移至 @runwork/session-core
...
// extractUserText 已迁移至 @runwork/session-core
...
// isUserInputMessage 已迁移至 @runwork/session-core
...
// AssistantTurn / MessageGroup 类型已迁移至 @runwork/session-core

// groupIntoTurns / mergeAdjacentSameModelTurns 已迁移至 @runwork/session-core
...
// stripScheduledRunMarker 已迁移至 @runwork/session-core（本文件从该包 import 使用）
...
function ScheduledRunBadge(): React.ReactElement {
  ...
  return (
    <button
      ...
      title="来自 RunWork 定时任务，点击查看设置"
    >
      <Clock className="size-3" />
      <span>来自 RunWork 定时任务</span>
    </button>
  )
}
...
// getGroupPreview 已迁移至 @runwork/session-core（本文件从该包 import 并 re-export）
