...
describe('updateSelectedModels - 模型勾选更新', () => {
  test('Given 空勾选列表 When 更新为部分勾选 Then selectedModelIds 正确替换', () => {
    const config: LegisConfig = { selectedModelIds: [], defaultModelId: '' }
...
describe('updateDefaultModel - 默认模型更新', () => {
  test('Given 无默认模型 When 设置默认模型 Then defaultModelId 正确', () => {
    const config: LegisConfig = { selectedModelIds: ['a', 'b'], defaultModelId: '' }
...
describe('loadLegisConfig - 从 localStorage 加载', () => {
  ...
  test('Given localStorage 有有效数据 When 加载 Then 返回存储的配置', async () => {
    localStorageMock.setItem('legis-config', JSON.stringify({
  ...
  test('Given localStorage 数据损坏（非法 JSON）When 加载 Then 返回默认配置不报错', async () => {
    localStorageMock.setItem('legis-config', '{invalid json')
    const config = await loadLegisConfig()
    expect(config.selectedModelIds).toEqual([])
    expect(config.defaultModelId).toBe('')
  })
...
describe('saveLegisConfig - 持久化到 localStorage', () => {
  test('Given 配置对象 When 保存 Then localStorage 含正确 JSON', async () => {
    const config: LegisConfig = {
      selectedModelIds: ['a', 'b'],
      defaultModelId: 'a',
    }
    await saveLegisConfig(config)
    const raw = localStorageMock.getItem('legis-config')
