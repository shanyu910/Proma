 ...
 *
 * 背景：
 * 登录流程会通过 IPC 写磁盘（channels.json、settings.json、legis-config），
...
export async function refreshStateAfterLogin(store: JotaiStore): Promise<void> {
  ...
  // 3. 刷新 legisConfigAtom（ModelSelector 按其中的 selectedModelIds 过滤官方渠道模型）
  const freshConfig = await loadLegisConfig()
  store.set(legisConfigAtom, freshConfig)
}
