...
// Mock window.electronAPI.runworkSK（clearSK/setSK 走 IPC）
const runworkSKMock = {
...
beforeEach(() => {
  runworkSKMock.clearSK.mockClear()
  runworkSKMock.setSK.mockClear()
})

// 替换全局 window.electronAPI
globalThis.window = {
  electronAPI: { runworkSK: runworkSKMock },
...
describe('clearSK - 清除主进程 SK', () => {
  test('Given 调用 clearSK Then 通过 IPC 清除主进程内存 SK', async () => {
    ...
    await clearSK()
    expect(runworkSKMock.clearSK).toHaveBeenCalledTimes(1)
  })
})
...
describe('fetchModelConfigData - 拉取模型配置', () => {
  test('Given active 状态 + 有效 provider When 拉取 Then 返回 config + SK 同步到主进程', async () => {
    ...
    // SK 应该通过 IPC 同步到主进程
    expect(runworkSKMock.setSK).toHaveBeenCalledWith('sk-test-123')
  })
