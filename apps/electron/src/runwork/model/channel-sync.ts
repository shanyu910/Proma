 ...
 *
 * 通过主进程专用 IPC 'runwork:upsert-official-channel' 写入官方渠道，
 * 用固定 ID 'runwork-official'（不走 createChannel 的 UUID 生成）。
 *
...
export const OFFICIAL_CHANNEL_ID = 'runwork-official'

/**
 * 把 model-config 同步到 channels.json
 *
 * @param config model-config 响应数据
 * @returns 官方渠道 ID（固定 'runwork-official'），失败为 null
 */
export async function syncModelConfigToChannels(config: ModelConfig): Promise<string | null> {
  ...
  try {
    ...
    const channelId = await window.electronAPI.runworkChannel.upsertOfficial({
    ...
    console.error('[RunWork] 同步 model-config 到 channels.json 失败:', error)
    return null
  }
}
