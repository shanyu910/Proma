 ...
 *
 * 由渲染进程登录后通过 IPC 'runwork:set-sk' 注入。
...
export async function handleSecureTokenGet(): Promise<string | null> {
  ...
  if (!safeStorage.isEncryptionAvailable()) {
    console.warn('[RunWork Auth] safeStorage 加密不可用')
    cacheLoaded = true
    return null
  }
  ...
  try {
    ...
    //
    // 方案：~/.runwork-dev/auth-token.enc（safeStorage 加密后的 base64 文本）
...
export async function handleSecureTokenSet(_: unknown, token: string): Promise<void> {
  if (!safeStorage.isEncryptionAvailable()) {
    console.warn('[RunWork Auth] safeStorage 加密不可用，Token 未持久化')
    cachedToken = token
    cacheLoaded = true
    return
  }
...
export async function handleSecureTokenClear(): Promise<void> {
  try {
    ...
    console.error('[RunWork Auth] 清除 Token 文件失败:', error)
  }
  cachedToken = null
  cacheLoaded = true
}
 ...
 * 获取加密 Token 文件路径
 *
 * 复用 RunWork 的配置目录逻辑（~/.runwork 或 ~/.runwork-dev）。
...
async function getSecureTokenFilePath(): Promise<string> {
  ...
  // 判断开发模式：与 config-paths.ts 的 RUNWORK_DEV 逻辑一致
  const isDev = process.env.RUNWORK_DEV === '1' || !appIsPackaged()
  const configDir = isDev ? '.runwork-dev' : '.runwork'
