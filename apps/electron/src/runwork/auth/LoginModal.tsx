  ...
  const handleLogin = async (): Promise<void> => {
    ...
    if (result.success && result.token && result.user) {
      ...
      if (modelConfig) {
        ...
        // 刷新全部相关 atom（channelsAtom + agentChannelIdAtom + runworkConfigAtom）
    ...
    <Dialog open={modal.open} onOpenChange={handleOpenChange}>
      <DialogContent className="!max-w-none w-[340px] p-6 gap-4" aria-describedby={undefined}>
        ...
        <DialogHeader>
          ...
          <p className="text-xs text-muted-foreground">RunWork</p>
        </DialogHeader>
        ...
        <div className="space-y-1">
          <label htmlFor="runwork-login-email" className="text-xs font-medium text-foreground/60">
            邮箱
          </label>
          <input
            id="runwork-login-email"
            type="email"
            value={email}
        ...
        {/* 密码 */}
        <div className="space-y-1">
          <label htmlFor="runwork-login-password" className="text-xs font-medium text-foreground/60">
            密码
          </label>
          <div className="relative">
            <input
              id="runwork-login-password"
