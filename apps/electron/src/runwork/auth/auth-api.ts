 ...
 * 认证 API 调用
 *
 * 对接 RunWork-Server 的 /auth/* 接口。
 * 服务器地址从 .env 的 VITE_RUNWORK_SERVER_URL 读取。
 *
...
/**
 * RunWork-Server 地址（构建期注入，来自 .env）
 *
 * 开发时 Vite 从 apps/electron/.env 读取。
 * fallback 为接口验证时确认的地址。
 */
const SERVER_URL: string =
  import.meta.env.VITE_RUNWORK_SERVER_URL || 'http://14.103.216.135:31006'
...
export async function checkSession(
  token: string,
): Promise<{ valid: boolean; user?: RunWorkUser; code?: string }> {
  try {
...
/**
 * 获取 RunWork-Server 地址（供其他模块使用，如 model-config）
 */
...
export async function updateProfile(
  token: string,
  fullName: string,
): Promise<{ success: boolean; user?: RunWorkUser; error?: string }> {
  try {
