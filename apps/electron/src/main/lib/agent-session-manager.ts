 ...
 * - 会话索引：~/.runwork/agent-sessions.json（轻量元数据）
 * - 消息存储：~/.runwork/agent-sessions/{id}.jsonl（JSONL 格式，逐行追加）
 *
...
import { migratePermissionMode } from '@runwork/shared'
import { getConversationMessages } from './conversation-manager'
// 旧格式 → SDKMessage 的转换逻辑下沉到 @runwork/session-core 作为唯一真源，避免主进程与渲染层各存一份。
import { convertLegacyMessage } from '@runwork/session-core'
...
/**
 * convertLegacyMessage 已迁移至 @runwork/session-core（本文件从该包 import 使用）。
 */
 ...
 *
 * SDK session JSONL（~/.runwork/sdk-config/projects/...）中的消息都带有 uuid，
