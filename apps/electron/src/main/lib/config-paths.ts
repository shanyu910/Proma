 ...
 * 配置路径工具
 *
 * 管理 RunWork 应用的本地配置文件路径。
 * 所有用户配置存储在 ~/.runwork/ 目录下。
 */
 ...
 * 获取配置目录名称
 *
 * 开发模式下返回 '.runwork-dev'，正式版本返回 '.runwork'。
 *
 * 检测优先级：
 * 1. RUNWORK_DEV=1 环境变量（显式覆盖）
...
export function getConfigDirName(): string {
  if (_configDirName === undefined) {
    if (process.env.RUNWORK_DEV === '1') {
      _configDirName = '.runwork-dev'
    } else {
      try {
        const { app } = require('electron')
        _configDirName = app.isPackaged ? '.runwork' : '.runwork-dev'
      } catch {
        _configDirName = '.runwork'
      }
    }
    const mode = _configDirName === '.runwork-dev' ? '开发模式' : '正式版本'
 ...
 * 获取配置目录路径
 *
 * 开发模式返回 ~/.runwork-dev/，正式版本返回 ~/.runwork/。
 * 如果目录不存在则自动创建。
 */
 ...
 * 获取对话索引文件路径
 *
 * @returns ~/.runwork/conversations.json
 */
 ...
 *
 * @param id 对话 ID
 * @returns ~/.runwork/conversations/{id}.jsonl
 */
 ...
 * @returns ~/.runwork/attachments/{conversationId}/
 */
 ...
 * @returns 完整路径 ~/.runwork/attachments/{conversationId}/{uuid}.ext
 */
 ...
 * 获取系统默认 App 探测缓存路径
 *
 * @returns ~/.runwork/default-apps.json
 */
 ...
 * 获取用户档案文件路径
 *
 * @returns ~/.runwork/user-profile.json
 */
 ...
 * 获取代理配置文件路径
 *
 * @returns ~/.runwork/proxy-settings.json
 */
 ...
 * 获取系统提示词配置文件路径
 *
 * @returns ~/.runwork/system-prompts.json
 */
 ...
 * 获取 Chat 工具配置文件路径
 *
 * @returns ~/.runwork/chat-tools.json
 */
 ...
 * 获取 Agent 会话索引文件路径
 *
 * @returns ~/.runwork/agent-sessions.json
 */
 ...
 * 如果目录不存在则自动创建。
 *
 * @returns ~/.runwork/agent-sessions/
 */
 ...
 *
 * @param id 会话 ID
 * @returns ~/.runwork/agent-sessions/{id}.jsonl
 */
 ...
 * 获取 Agent 工作区索引文件路径
 *
 * @returns ~/.runwork/agent-workspaces.json
 */
 ...
 * 如果目录不存在则自动创建。
 *
 * @returns ~/.runwork/agent-workspaces/
 */
 ...
 *
 * @param slug 工作区 slug
 * @returns ~/.runwork/agent-workspaces/{slug}/
 */
 ...
 *
 * @param slug 工作区 slug
 * @returns ~/.runwork/agent-workspaces/{slug}/mcp.json
 */
 ...
 *
 * @param slug 工作区 slug
 * @returns ~/.runwork/agent-workspaces/{slug}/skills/
 */
 ...
 *
 * @param slug 工作区 slug
 * @returns ~/.runwork/agent-workspaces/{slug}/workspace-files/
 */
 ...
 * @returns ~/.runwork/agent-workspaces/{slug}/{sessionId}/
 */
 ...
 *
 * @param slug 工作区 slug
 * @returns ~/.runwork/agent-workspaces/{slug}/skills-inactive/
 */
 ...
 *
 * @returns ~/.runwork/default-skills/
 */
...
export function getBundledCliPath(): string | undefined {
  ...
  const binName = process.platform === 'win32' ? 'runwork.exe' : 'runwork'
...
/**
 * 从 app bundle 同步默认 Skills 到 ~/.runwork/default-skills/
 *
 ...
 * 获取微信长轮询同步游标路径
 *
 * @returns ~/.runwork/wechat-sync.json
 */
 ...
 * 获取微信聊天绑定持久化路径
 *
 * @returns ~/.runwork/wechat-bindings.json
 */
 ...
 *
 * @returns ~/.runwork/dingtalk-bindings-{botId}.json
 */
 ...
 * 获取飞书聊天绑定持久化路径
 *
 * @returns ~/.runwork/feishu-bindings.json
 */
 ...
 *
 * @returns ~/.runwork/feishu-bindings-{botId}.json
 */
 ...
 *
 * @returns ~/.runwork/feishu-metadata-{botId}.json
 */
 ...
 * 而不是用户的 ~/.claude.json，实现 RunWork 与 Claude Code CLI 的配置隔离。
 *
 * 如果目录不存在则自动创建。
 *
 * @returns ~/.runwork/sdk-config/
 */
 ...
 * 获取 Scratch Pad 文件路径
 *
 * @returns ~/.runwork/scratch-pad.md
 */
 ...
 *
 * @returns ~/.runwork/automations.json
 */
