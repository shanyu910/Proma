 ...
 * 获取配置目录名称
 *
 * 开发模式下返回 '.legis-dev'，正式版本返回 '.legis'。
 *
 * 检测优先级：
 * 1. LEGIS_DEV=1 环境变量（显式覆盖）
...
export function getConfigDirName(): string {
  if (_configDirName === undefined) {
    if (process.env.LEGIS_DEV === '1') {
      _configDirName = '.legis-dev'
    } else {
      try {
        const { app } = require('electron')
        _configDirName = app.isPackaged ? '.legis' : '.legis-dev'
      } catch {
        _configDirName = '.legis'
      }
    }
    const mode = _configDirName === '.legis-dev' ? '开发模式' : '正式版本'
 ...
 * 获取对话索引文件路径
 *
 * @returns ~/.legis/conversations.json
 */
 ...
 *
 * @param id 对话 ID
 * @returns ~/.legis/conversations/{id}.jsonl
 */
 ...
 * @returns ~/.legis/attachments/{conversationId}/
 */
 ...
 * @returns 完整路径 ~/.legis/attachments/{conversationId}/{uuid}.ext
 */
 ...
 * 获取系统默认 App 探测缓存路径
 *
 * @returns ~/.legis/default-apps.json
 */
 ...
 * 获取用户档案文件路径
 *
 * @returns ~/.legis/user-profile.json
 */
 ...
 * 获取代理配置文件路径
 *
 * @returns ~/.legis/proxy-settings.json
 */
 ...
 * 获取系统提示词配置文件路径
 *
 * @returns ~/.legis/system-prompts.json
 */
 ...
 * 获取 Agent 会话索引文件路径
 *
 * @returns ~/.legis/agent-sessions.json
 */
 ...
 *
 * @param id 会话 ID
 * @returns ~/.legis/agent-sessions/{id}.jsonl
 */
 ...
 * 获取 Agent 工作区索引文件路径
 *
 * @returns ~/.legis/agent-workspaces.json
 */
 ...
 * 如果目录不存在则自动创建。
 *
 * @returns ~/.legis/agent-workspaces/
 */
 ...
 *
 * @param slug 工作区 slug
 * @returns ~/.legis/agent-workspaces/{slug}/
 */
 ...
 *
 * @param slug 工作区 slug
 * @returns ~/.legis/agent-workspaces/{slug}/mcp.json
 */
 ...
 *
 * @param slug 工作区 slug
 * @returns ~/.legis/agent-workspaces/{slug}/skills/
 */
 ...
 *
 * @param slug 工作区 slug
 * @returns ~/.legis/agent-workspaces/{slug}/workspace-files/
 */
 ...
 * @returns ~/.legis/agent-workspaces/{slug}/{sessionId}/
 */
 ...
 *
 * @param slug 工作区 slug
 * @returns ~/.legis/agent-workspaces/{slug}/skills-inactive/
 */
...
export function getBundledCliPath(): string | undefined {
  ...
  const binName = process.platform === 'win32' ? 'legis.exe' : 'legis'
...
/**
 * 从 app bundle 同步默认 Skills 到 ~/.legis/default-skills/
 *
 ...
 * 获取微信长轮询同步游标路径
 *
 * @returns ~/.legis/wechat-sync.json
 */
 ...
 * 获取微信聊天绑定持久化路径
 *
 * @returns ~/.legis/wechat-bindings.json
 */
 ...
 *
 * @returns ~/.legis/dingtalk-bindings-{botId}.json
 */
 ...
 * 获取飞书聊天绑定持久化路径
 *
 * @returns ~/.legis/feishu-bindings.json
 */
 ...
 *
 * @returns ~/.legis/feishu-bindings-{botId}.json
 */
 ...
 *
 * @returns ~/.legis/feishu-metadata-{botId}.json
 */
 ...
 * 而不是用户的 ~/.claude.json，实现 Legis 与 Claude Code CLI 的配置隔离。
 *
 * 如果目录不存在则自动创建。
 *
 * @returns ~/.legis/sdk-config/
 */
 ...
 *
 * @returns ~/.legis/automations.json
 */
