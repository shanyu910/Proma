...
export interface ClaudeAgentQueryOptions extends AgentQueryInput {
  ...
  maxTurns?: number
  /** SDK 权限模式（RunWork 当前三种模式直接映射 SDK 原生模式） */
  ...
  /** 仅使用 RunWork 显式传入的 MCP 配置，避免 SDK 从其它来源发现额外 MCP */
...
const FRIENDLY_ERROR_MESSAGES: Array<{ pattern: RegExp; message: string }> = [
  {
    ...
    message: '请检查是否选择了正确的 RunWork 供应渠道和模型',
  },
  {
...
export function mapSDKErrorToTypedError(
  ...
  const isInvalidChannelOrModel = /请检查是否选择了正确的 RunWork 供应渠道和模型/.test(mapped.message)
