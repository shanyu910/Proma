 ...
 * - 消息路由到 RunWork Agent，通过 sessionWebhook 回复
 *
...
import { DINGTALK_IPC_CHANNELS } from '@runwork/shared'
...
class DingTalkBridge {
  ...
  private async processRobotMessage(msg: DWClientDownStream): Promise<void> {
    ...
    if (!binding) {
      await this.replyTextViaWebhook(data.sessionWebhook, '请先在 RunWork 设置中选择 Agent 渠道。')
      return
    }
