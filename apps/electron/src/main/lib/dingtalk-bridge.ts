 ...
 * - 消息路由到 Legis Agent，通过 sessionWebhook 回复
 *
...
import { DINGTALK_IPC_CHANNELS } from '@legis/shared'
...
class DingTalkBridge {
  ...
  private async processRobotMessage(msg: DWClientDownStream): Promise<void> {
    ...
    if (!binding) {
      await this.replyTextViaWebhook(data.sessionWebhook, '请先在 Legis 设置中选择 Agent 渠道。')
      return
    }
