 ...
 * - 工作区索引：~/.runwork/agent-workspaces.json（轻量元数据）
 * - 工作区目录：~/.runwork/agent-workspaces/{slug}/（Agent 的 cwd）
 */
...
import { inferMcpTransportType, normalizeMcpTransportType } from '@runwork/shared'
import type { AgentWorkspace, WorkspaceMcpConfig, SkillMeta, SkillImportSource, OtherWorkspaceSkillsGroup, WorkspaceCapabilities, SkillFileNode, SkillFileContent, WorkspaceMemorySummary } from '@runwork/shared'
...
/** 将 ~/.runwork/default-skills/ 的内容逐个复制到工作区 skills/ 目录 */
...
interface WorkspaceConfig {
  ...
  worktreeRepos?: import('@runwork/shared').WorkspaceWorktreeRepo[]
}
 ...
 */
export async function getWorktreeRepos(workspaceSlug: string): Promise<import('@runwork/shared').WorkspaceWorktreeRepo[]> {
  const config = readWorkspaceConfig(workspaceSlug)

  // repoPath 归一化后去重
  const byPath = new Map<string, import('@runwork/shared').WorkspaceWorktreeRepo>()
...
export function addWorktreeRepo(workspaceSlug: string, repo: import('@runwork/shared').WorkspaceWorktreeRepo): import('@runwork/shared').WorkspaceWorktreeRepo[] {
  const config = readWorkspaceConfig(workspaceSlug)
...
export function removeWorktreeRepo(workspaceSlug: string, repoPath: string): import('@runwork/shared').WorkspaceWorktreeRepo[] {
  const config = readWorkspaceConfig(workspaceSlug)
  const existing = config.worktreeRepos ?? []
