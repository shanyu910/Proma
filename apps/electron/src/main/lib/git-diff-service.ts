...
import type { ChangedFileEntry, UnstagedChangesResult, UntrackedFileEntry } from '@runwork/shared'
import { normalizePathForCompare } from '@runwork/shared'
import type { ChangeSource, ChangedFileStatus } from '@runwork/shared'
 ...
 */
export async function listWorktrees(repoPath: string): Promise<import('@runwork/shared').WorktreeInfo[]> {
  ...
  const worktrees: import('@runwork/shared').WorktreeInfo[] = []
...
export async function getWorktreeChanges(
...
): Promise<import('@runwork/shared').UnstagedChangesResult> {
  ...
  const allFiles: import('@runwork/shared').ChangedFileEntry[] = []
  const fileMap = new Map<string, import('@runwork/shared').ChangedFileEntry>()
  ...
  if (committedStatus) {
    for (const line of committedStatus.split('\n').filter(Boolean)) {
      ...
      let status: import('@runwork/shared').ChangedFileStatus
      let filePath: string
      ...
      const entry: import('@runwork/shared').ChangedFileEntry = {
        filePath,
        status,
        additions: stats.additions,
        deletions: stats.deletions,
  ...
  // 3. 新文件（未追踪）
  const untrackedFiles: import('@runwork/shared').UntrackedFileEntry[] = []
  const untrackedOutput = await runGitCommand(['ls-files', '--others', '--exclude-standard'], gitRoot)
  if (untrackedOutput) {
    for (const rel of untrackedOutput.split('\n').filter(Boolean)) {
      if (!fileMap.has(rel)) {
