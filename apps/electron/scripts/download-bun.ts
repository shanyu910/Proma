...
import type { PlatformArch, BunDownloadInfo } from '@legis/shared'
...
async function downloadFile(url: string, destPath: string): Promise<void> {
  ...
  const response = await fetch(url, {
    headers: {
      'User-Agent': 'Legis-Build-Script/1.0',
    },
    redirect: 'follow',
  })
...
async function getBunVersion(): Promise<string> {
  ...
  const version = pkg.legis?.bun?.version
  if (!version) {
    throw new Error('package.json 中未配置 legis.bun.version')
  }
