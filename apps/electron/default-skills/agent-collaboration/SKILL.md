...
description: Legis 协作子 Agent Skill。用户任务复杂、可并行、长耗时、需要多个独立方向同时推进、需要真实可见进展、需要保留完整子任务记录，或用户明确说“开几个 Agent/多会话/一起协作/并行处理/spawn 子 Agent”时触发。用于判断是否调用 Legis 内置 collaboration 工具创建真实子会话。简单搜索、短调研、单文件修改、一次性代码审查优先使用 SDK 内置 SubAgent，不要创建真实 Legis 子会话。
group: legis
version: "1.0.2"
---

# Legis Agent Collaboration

你负责判断何时把复杂任务交给 Workflow、SDK SubAgent，或拆给真实可见的 Legis 协作子 Agent 会话。

Legis 已提供内置 `collaboration` MCP 工具。你必须通过这些工具创建、等待、查看和停止协作子会话，不要用 Bash、脚本或直接修改 `~/.legis/agent-sessions.json` 的方式创建会话。
...
### Scenario 2：独立并行任务应使用 Legis 协作编排
...
Then Agent 应使用 SDK SubAgent 或普通搜索工具，不创建真实 Legis 子会话。
