...
description: RunWork 协作子 Agent Skill。当需要并行探索多个方向（多样性探索）、对抗性审查验证已有方案、或多个长耗时独立任务需要真实可见的子会话时触发。用于判断是否以及如何调用 RunWork 内置 collaboration 工具创建协作子会话。简单搜索、短调研、单文件修改、一次性代码审查优先使用 SDK SubAgent。
group: runwork
version: “1.1.0”
---

# RunWork Agent Collaboration

你负责判断何时把复杂任务交给 Workflow、SDK SubAgent，或拆给真实可见的 RunWork 协作子 Agent 会话。

RunWork 已提供内置 `collaboration` MCP 工具。你必须通过这些工具创建、等待、查看和停止协作子会话，不要用 Bash、脚本或直接修改 `~/.runwork/agent-sessions.json` 的方式创建会话。
...
### Scenario 2：独立并行任务应使用 RunWork 协作编排
...
Then Agent 应使用 SDK SubAgent 或普通搜索工具，不创建真实 RunWork 子会话。
